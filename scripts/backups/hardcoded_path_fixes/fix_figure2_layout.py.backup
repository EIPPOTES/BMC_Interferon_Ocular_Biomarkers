...
def generate_figure2_optimized():
    """生成优化排版的Figure 2"""
    ...
    for idx, (col, title, unit) in enumerate(indicators):
        ...
        # 计算统计量
        median_mdd = mdd_data.median()
        median_ctrl = control_data.median()
        ...
        # n值放在箱图上方
        ax.text(1, y_max - y_range*0.02, f'n={n_mdd}', 
               ha='center', fontsize=9, fontweight='bold',
               bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
        ax.text(2, y_max - y_range*0.02, f'n={n_control}', 
               ha='center', fontsize=9, fontweight='bold',
               bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
    ...
    note_text = (
        ...
        "Outliers shown as individual points. n values indicate actual number of eyes with valid data. "
    ...
    # 保存
    output_path = '/mnt/c/Users/CUI/Desktop/投稿版/Figures_修订版/Figure2_Group_Comparison_优化版.png'
    ...
    print("\n排版优化:")
    print("  ✅ 增大图间距 (hspace=0.35, wspace=0.25)")
    print("  ✅ n值添加白色背景框，避免重叠")
    print("  ✅ 调整y轴范围，给标注留足够空间")
    print("  ✅ 优化字体大小和间距")
    print("\n各指标n值:")
    for col, title, unit in indicators:
        mdd_data = df[(df['分组'] == '抑郁症') & (df[col].notna())][col]
        control_data = df[(df['分组'] == '健康对照') & (df[col].notna())][col]
        print(f"  {title}: MDD n={len(mdd_data)}, Control n={len(control_data)}")

if __name__ == "__main__":
    print("="*60)
    print("生成优化排版的Figure 2")
    print("="*60)
    generate_figure2_optimized()
    print("\n" + "="*60)
    print("✅ Figure 2排版优化完成!")
    print("="*60)